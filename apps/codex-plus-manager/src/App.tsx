...
const routes: Array<{ id: Route; label: string; icon: LucideIcon }> = [
  ...
  { id: "proxy", label: "代理服务�?, icon: ShieldCheck },
  ...
  const switchRelayProfile = async (next: BackendSettings, previousActiveRelayId = settingsForm.activeRelayId) => {
    ...
    try {
      ...
      if (!isSuccessStatus(result.status)) {
        ...
        showNotice("供应商切�?, result.message, result.status);
        return;
      }
      ...
      showNotice("供应商切�?, relayProfileModeSwitchedText(currentSelected), result.status);
    } finally {
    ...
    () => ({
      ...
      chooseImageOverlayPath: async () => {
        ...
        try {
          ...
          const message = error instanceof Error ? error.message : String(error);
          showNotice("图片覆盖�?, `打开选择器失败：${message}`, "failed");
          return;
        }
        if (typeof selected === "string" && selected.trim()) {
...
function OverviewScreen({
  ...
  return (
    <>
      ...
      <Panel>
        <CardHead title="代理服务器启动状�? detail={overview?.logs_path ?? "暂无状态文�?} />
        <CardContent>
        ...
        <CardContent>
          ...
          <label className="switch-row">
            ...
            <span>
              ...
              <small>默认关闭；开启后启动 Codex 时会自动保留官方 Computer Use 插件所需�?config.toml、bundled 插件�?notify 配置�?/small>
            </span>
          </label>
          ...
          <div className="feature-switch-grid">
            ...
            <FeatureToggle title="原生菜单栏位�? detail="�?LDCodex 菜单插入 Codex 顶部原生菜单栏�? checked={form.codexAppNativeMenuPlacement} disabled={!masterEnabled} onChange={(value) => setEnhanceFlag("codexAppNativeMenuPlacement", value)} />
          </div>
          <div className="hint-line">
...
function MaintenanceScreen({
  ...
  return (
    <>
      ...
      <Panel>
        <CardHead title="自动接管" detail="Watcher 用于保持 LDCodex 接管状�? />
        <CardContent>
          <Toolbar>
...
function ProxyScreen({
  ...
  return (
    <>
      <Panel>
        <CardHead title="代理服务�? detail="代理服务器启动状态与配置" />
        <CardContent>
          <LatestLaunch status={overview?.latest_launch ?? null} />
          <Toolbar>
            <Button onClick={() => void actions.launch()}>
...
function AboutScreen({
  ...
  return (
    <>
      <Panel>
        <CardHead title="关于 LDCodex" detail="本地 Codex 增强和管理工�? />
        <CardContent>
          <div className="metric-list">
      ...
      <Panel>
        ...
        <CardContent>
          ...
          <div className="settings-block">
            ...
            <Field label={`透明�?${form.codexAppImageOverlayOpacity}%`}>
              <Input
                min={1}
                max={100}
                type="range"
    ...
    <div className="relay-profile-editor">
      <div className="relay-editor-head">
        ...
        {isNew ? null : (
          ...
          >
            {actions.relaySwitching ? "切换�? : profile.id === form.activeRelayId ? "使用�? : "设为当前"}
          </Button>
        )}
      </div>
      {isNew ? (
