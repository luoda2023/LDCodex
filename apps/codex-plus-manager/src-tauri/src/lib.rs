...
pub fn run() {
    ...
    // LDCodex：在线更新已停用，启动不再带 showUpdate 参数。
    ...
    let app_result = tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            {
                // LDCodex：Windows 使用自绘标题栏（随主题/页面情景变色）。
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::freebuff_status,
            commands::freebuff_list_models,
            ...
            commands::list_workbuddy_sessions,
            commands::soft_delete_workbuddy_sessions,
            commands::restore_workbuddy_sessions,
            commands::purge_workbuddy_sessions,
...
pub fn focus_existing_manager_window() {
    #[cfg(windows)]
    {
        ...
        for process in codex_plus_core::windows_enumerate_processes() {
            ...
            if process
                .exe_file
                .eq_ignore_ascii_case("LDCodexManager.exe")
            {
