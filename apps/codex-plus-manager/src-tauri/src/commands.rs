...
fn load_overview_sync() -> CommandResult<OverviewPayload> {
    ...
    ok(
        ...
        OverviewPayload {
            ...
            zcode_shortcut: shortcut_state(entrypoints.zcode_shortcut),
            latest_launch,
    ...
    let Ok((codex_app_path, entrypoints, latest_launch)) = payload else {
        return failed(
            ...
            OverviewPayload {
                ...
                zcode_shortcut: path_state(None),
                latest_launch: None,
                current_version: codex_plus_core::version::VERSION.to_string(),
                update_status: "not_checked".to_string(),
                settings_path: codex_plus_core::paths::default_settings_path()
...
/// 查找 node.exe 完整路径（代替旧版 find_npx）
fn find_node() -> Option<PathBuf> {
    ...
    // 3. 环境变量 NODE_PATH
    if let Some(np) = std::env::var_os("NODE_PATH") {
        let p = PathBuf::from(&np);
        if p.is_file() { return Some(p); }
        let p = p.join("node.exe");
        if p.is_file() { return Some(p); }
    }
    None
}
...
/// 注入 ZCode 插件（复制脚本到 ZCode 安装目录）
///
/// 使用项目内安装的 @electron/asar 包，不依赖 npx 网络下载。
/// asar CLI 路径 = <current_exe_dir>/../node_modules/@electron/asar/bin/asar.mjs
#[tauri::command]
pub fn inject_zcode_plugin() -> CommandResult<Value> {
    ...
    if !asar_path.exists() {
        return failed(
            ...
            json!({"asar": asar_path.to_string_lossy().to_string()}),
        );
    }
