...
function sharedDataDir() {
  return path.join(appSupport, 'LDCodex-WorkBuddy');
}

const PROFILES = {
  'workbuddy-cn': {
    id: 'workbuddy-cn', name: 'WorkBuddy', appName: 'LDCodex', region: 'cn', kind: 'workbuddy', mode: 'agents',
    ...
    // CDP 端口只能通过环境变量 WORKBUDDY_REMOTE_DEBUGGING_PORT 注入：
    // 客户端主进程在 ready 前读取它并 appendSwitch('remote-debugging-port')，
    // 而命令行形式的 --remote-debugging-port 会被其 argv 校验直接拒绝（bad option）后退出。
    cdp: { mode: 'environment' },
    targetHints: ['workbuddy'],
  },
  'workbuddy-ai': {
    id: 'workbuddy-ai', name: 'WorkBuddy AI', appName: 'LDCodex', region: 'intl', kind: 'workbuddy', mode: 'agents',
    ...
    capabilities: { accounts: true, sessions: true, models: true, stashPrompt: true, theme: true, checkin: true },
    // 与国际版同理：CDP 端口走环境变量，命令行参数会被拒绝（bad option）。
    cdp: { mode: 'environment' },
