 ...
 *   WBSWITCH_DATA_DIR    备份数据目录（默认 ~/Library/Application Support/LDCodex）
...
// LDCodex：这里必须用容错版。上游在 require 阶段直接调用会抛错的 detectWindowsPrivilege，
// 只要 PowerShell 不可用（组策略/杀软/超时）整个增强服务就起不来。探测不到一律按普通权限处理。
const DAEMON_PRIVILEGE = process.platform === 'win32'
  ? (process.env.WBSWITCH_NATIVE_LAUNCHER === '1' ? 'standard' : detectWindowsPrivilegeSafe())
  : 'standard';
...
// 0.6.8：关于页精简（只留版本 + GitHub 链接），仓库改为 github.com/，去掉 logo/原理/平台/运行时
...
//       applyUpdate 现改为在安装前调用 extractAppFromDmg 解出 LDCodex.app（幂等），
//       解包函数亦增强（清理残留挂载点、只读挂载、校验 dmg 内存在 LDCodex.app）
// 1.0.5（修复版打包）：修复 Windows 自动更新三大卡死根因，让「更新已启动，LDCodex 即将重启」
//     到真正更新完成：
...
//         macOS 不再把更新目标硬编码为 /Applications/LDCodex.app
...
// 1.1.16：元素检查器改为 LDCodex 插件内弹窗，支持 DOM 树、悬停高亮和重叠元素浏览，不再提供独立页面。
// 1.1.24：6 号官方壁纸替换为新默认图；消息导航与机器人瞳孔改为悬浮毛玻璃。
// 1.1.25：内置官方壁纸更新时刷新数据目录旧副本；新 profile 默认启用 LDCodex 壁纸主题。
...
// 1.1.39：账号脱敏状态按 profile 持久化；LDCodex 触发页面重载后在主执行上下文创建时提前注入。
// 1.1.40：跨账号重载跟随新主 frame，并在会话自动复制占用事件循环前等待组件实际挂载。
// 1.1.41：后台会话自动复制在文件边界让出 I/O；账号重载期间暂停复制，优先完成组件挂载。
// 1.1.42：删除账号时同步清理旧版 LDLegacy 迁移源，避免重启后账号备份复活。
...
// Windows 安装目录（install.ps1 铺、launcher 用、更新替换目标），对应 macOS 的 /Applications/LDCodex.app
const LDCODEX_INSTALL_NAME = PROFILE.id === 'workbuddy-ai' ? 'LDCodex' : 'LDCodex';
const LDCODEX_DIR_WIN = process.env.WBSWITCH_APP_DIR || path.resolve(__dirname, '..');
 ...
 */
// LDCodex：已移除 LDCodex 的 GitHub Releases 在线更新链路。
const UPDATE_REPO = '';
const UPDATE_API = '';
...
function macLDCodexAppPath() {
  ...
  return `/Applications/${LDCODEX_INSTALL_NAME}.app`;
}
...
function httpsGet(url, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    const req = mod.get(url, { headers: { 'User-Agent': 'LDCodex/' + DAEMON_VERSION, Accept: 'application/vnd.github+json' } }, (res) => {
      const chunks = [];
  ...
  return httpsGet(UPDATE_API)
    .then(({ status, body }) => {
      ...
      const profileAsset = PROFILE.id === 'workbuddy-ai'
        ? /^(?:LDCodex-AI-Setup-|LDCodex-AI-).*\.(?:exe|zip|dmg)$/i
        : /^LDCodex-(?!AI-)(?:Setup-|).*\.(?:exe|zip|dmg)$/i;
      const profileSetup = PROFILE.id === 'workbuddy-ai'
        ? /^LDCodex-AI-Setup-\d+\.\d+\.\d+\.exe$/i
        : /^LDCodex-Setup-\d+\.\d+\.\d+\.exe$/i;
      const profileZip = PROFILE.id === 'workbuddy-ai'
        ? /^LDCodex-AI-\d+\.\d+\.\d+-win64\.zip$/i
        : /^LDCodex-\d+\.\d+\.\d+-win64\.zip$/i;
      const asset = IS_WIN
        ...
        : (assets.find((a) => profileAsset.test(a.name || '') && /\.dmg$/i.test(a.name || '')) ||
           assets.find((a) => /\.dmg$/i.test(a.name || '') && (PROFILE.id !== 'workbuddy-ai' || !/LDCodex-AI-/i.test(a.name || ''))) || null);
...
function downloadUpdateInternal() {
  ...
  const updatePrefix = PROFILE.id === 'workbuddy-ai' ? 'LDCodex-AI-' : 'LDCodex-';
  ...
  return new Promise((resolve, reject) => {
    ...
    mod.get(updateState.dmgUrl, { headers: { 'User-Agent': 'LDCodex/' + DAEMON_VERSION } }, (res) => {
    ...
    const req = mod.request(
      {
        ...
        headers: Object.assign(
          { 'User-Agent': 'LDCodex/' + DAEMON_VERSION, Accept: 'application/json' },
...
// 从 dmg 中解出 LDCodex.app 到 UPDATE_DIR（挂载→拷贝→卸载），返回 app 目录
function extractAppFromDmg(dmgPath) {
  const mountPoint = '/Volumes/LDCodex-update';
  const appPackageName = LDCODEX_INSTALL_NAME + '.app';
  ...
  return new Promise((resolve, reject) => {
    ...
    exec('hdiutil', ['detach', mountPoint, '-force'], () => {
      exec('hdiutil', ['attach', '-nobrowse', '-readonly', '-noverify', '-mountpoint', mountPoint, dmgPath], (err) => {
        ...
        const src = path.join(mountPoint, LDCODEX_INSTALL_NAME + '.app');
        if (!fs.existsSync(src)) {
          exec('hdiutil', ['detach', mountPoint, '-force'], () => reject(new Error(`dmg 中未找到 ${LDCODEX_INSTALL_NAME}.app`)));
          return;
        }
...
function applyUpdate() {
  ...
  if (IS_WIN) {
    ...
    const expectedAsset = PROFILE.id === 'workbuddy-ai'
      ? `LDCodex-AI-Setup-${updateState.latest}.exe`
      : `LDCodex-Setup-${updateState.latest}.exe`;
  ...
  const appPath = macLDCodexAppPath();
  const srcApp = path.join(UPDATE_DIR, LDCODEX_INSTALL_NAME + '.app');
  if (!fs.existsSync(scriptPath)) {
    const error = new Error('缺少 apply-update.sh');
    markAttemptFailure(error, 'preflight-error');
    return Promise.reject(error);
  }
  // 解出新应用：下载阶段只落了 .dmg，这里才把 LDCodex.app 从 dmg 解到 UPDATE_DIR（幂等：已解出则复用）
  const updatePrefix = PROFILE.id === 'workbuddy-ai' ? 'LDCodex-AI-' : 'LDCodex-';
...
function isCurrentWindowsDaemonProcess(pid) {
  ...
  // 先用 Node 原生接口做一次存在性判定：锁持有者的进程若已退出，必须立刻返回 false，
  // 否则下面的 PowerShell/CIM 查询一旦失败（超时、策略拦截、脚本缺失）就会走到
  // 「保守保留锁」分支，把僵尸锁判为有效，导致增强服务永远无法再次启动。
  try {
    process.kill(pid, 0);
  } catch (probeError) {
    const probeCode = probeError && probeError.code;
    // ESRCH=进程不存在；EINVAL=PID 非法。两者都说明锁已僵尸，可安全接管。
    if (probeCode === 'ESRCH' || probeCode === 'EINVAL') return false;
    // EPERM 等表示进程仍存在但无权限访问，继续走下面的身份校验。
  }
  ...
  try {
    const result = spawnSync('powershell', ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-Command', command], {
      // 同 queryWindowsWorkBuddyProcesses：本机 PowerShell 冷启动 5~7s，10s 阈值会随机超时。
...
const DAEMON_LOCK_FALLBACK_FILE = path.join(
  os.tmpdir(),
  'LDCodex-daemon-' + crypto.createHash('sha256').update(path.resolve(DATA_DIR)).digest('hex').slice(0, 16) + '.lock'
);
...
let pendingReloadInjection = null; // 仅对 LDCodex 主动触发的页面重载做一次主 frame 早期注入
  ...
  return new Promise((resolve) => {
    ...
    ws.onopen = () => {
      ...
      setTimeout(() => {
        ...
        // 这里重新应用，保证「LDCodex 主题=深色 / WorkBuddy 默认主题=浅色」在重启后仍生效）
  ...
  switch (method) {
    ...
    case 'Page.loadEventFired': {
      ...
      if (pendingReloadInjection) {
        ...
        suppressPageLoadInjectionForNavigation = 0;
        // 非 LDCodex 触发的刷新仍在页面加载完成后恢复组件。
        injectWidget('page-load').catch(() => {});
      }
      // 页面刷新后 WorkBuddy 回到官方浅色，重新应用已保存主题（LDCodex=深色 / 默认=浅色）
...
function queryWindowsWorkBuddyProcesses() {
  // 本机 PowerShell 冷启动实测 5~7s，而 tasklist 只要约 0.3s。
  // “该 profile 没有进程”是最常见路径（客户端没开），先用 tasklist 预检即可直接返回，
  // 避免每次操作都白等十几秒；只有真有进程时才走下面的严格 CIM 校验。
  const namePresent = [...PROFILE_BINARY_NAMES].some((name) => {
    try {
      const probe = spawnSync('tasklist', ['/FI', `IMAGENAME eq ${name}`, '/NH', '/FO', 'CSV'], {
        encoding: 'utf8', timeout: 8000, windowsHide: true,
      });
      return probe.status === 0 && /\.exe"/i.test(probe.stdout || '');
    } catch (_) {
      return true; // 探测异常时保守处理，交给下面的严格查询
    }
  });
  ...
  const result = spawnSync('powershell', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', command], {
    // 本机 PowerShell 冷启动实测 5~7s（AMSI/安全软件扫描），原 10s 阈值会随机超时；
    // 超时会被误判成“没有进程在运行”，导致跳过关闭旧实例。
    encoding: 'utf8', timeout: 30000, windowsHide: true,
  });
...
function resolveWorkBuddyBinary() {
  ...
  // 注意：powershell.exe -Command 只要最后一条命令写入了错误记录（例如查询并不存在的
  // 注册表键，即使带 -ErrorAction SilentlyContinue），退出码就是 1。原先这里用
  // execFileSync，会把这种“本来就查不到”的情况当成致命异常抛出，导致客户端未运行时的
  // 「休眠发现」整条链路直接中断（表现为：客户端没开时点重启必定失败）。
  // 这里只取 stdout，不因退出码非零而抛错。
  const psCmd = (cmd) => {
    const r = spawnSync('powershell', ['-NoProfile', '-Command', cmd], { encoding: 'utf8', timeout: 30000, windowsHide: true });
...
async function restoreWorkBuddyWindow(pid) {
  ...
  const source = [
    ...
    'public static class LDCodexWindowBridge {',
  ...
  const command = `Add-Type -TypeDefinition @'\n${source}\n'@; [LDCodexWindowBridge]::Restore(${Number(pid)})`;
  const encoded = Buffer.from(command, 'utf16le').toString('base64');
  // 这一步会现场 Add-Type 编译 C#，比普通 PowerShell 冷启动更慢；实测本机常超 10s。
  const result = await runCommand('powershell', ['-NoProfile', '-WindowStyle', 'Hidden', '-EncodedCommand', encoded], { timeoutMs: 30000 });
...
function revalidateWindowsWorkBuddyProcess(original, binary) {
  ...
  if (!current) {
    // 目标可能在两阶段之间自己退出了；这里返回 null 交给调用方跳过，
    // 不要中断整个退出流程（否则会误判成"无法退出"）。
    return null;
  }
...
async function quitWorkBuddy() {
  if (IS_WIN) {
    ...
    for (const process of processes) {
      ...
      if (result.error || result.code !== 0) {
        // Electron 主窗口通常不响应 WM_CLOSE，taskkill 会回"只能使用 /F 强制终止"。
        // 这里只记日志，交给下面的强制阶段处理——过早抛错会让整个重启卡死。
        log(`[cdp] 优雅结束 WorkBuddy PID=${process.ProcessId} 未生效，转入强制结束: ${result.error ? result.error.message : `exit=${result.code}`}`);
      }
    }
    ...
    for (const process of processes) {
      ...
      if (result.error || result.code !== 0) {
        forceFailures.push(`${process.ProcessId}: ${result.error ? result.error.message : `exit=${result.code}`}`);
      }
    }
    if (await waitForWorkBuddyExit(3000, binary)) return true;

    // 强制也失败了才算真失败（个别进程"已不存在"会返回非零，但那时上面已经确认退出了）。
    const detail = forceFailures.length ? `（强制结束失败：${forceFailures.join('; ')}）` : '';
    throw new Error(`无法以普通用户权限安全退出 WorkBuddy${detail}。请手动关闭该程序；若它以管理员身份运行，请先退出后再重试。登录信息未修改`);
  }
...
/** 探测 LDCodex.app 位置（macOS 专用：退出登录后打开它，由其 launcher 以 CDP 模式重启 WorkBuddy 并注入组件） */
function findLDCodexApp() {
  ...
  const cands = [
    ...
    path.join(__dirname, '..', 'LDCodex.app'),
...
/** 重新启动 WorkBuddy：macOS 优先走 LDCodex.app launcher；Windows 直接带 CDP 参数重启 exe */
function relaunchWorkBuddy() {
  return (async () => {
    ...
    const workDaddy = findLDCodexApp();
    if (workDaddy) {
      log(`[logout] 正在打开 LDCodex (${workDaddy})，由其 launcher 重启 WorkBuddy`);
    ...
    log(`[logout] 未找到 LDCodex.app，直接重新启动 WorkBuddy（带 CDP 端口 ${port}）`);
...
// 自动化运行前收拢 LDCodex 面板「窗口」，避免其 contenteditable/悬浮层与 WorkBuddy 原生
// composer 抢焦点或遮挡，导致任务把提示词键入到 LDCodex 面板输入框 / 点不到官方发送按钮
...
function automationPanelSetInputActive(active) {
  ...
  return cdpSend('Runtime.evaluate', {
    expression: `window.__wbsAutomationInputActive=${!!active};${active ? "window.dispatchEvent(new CustomEvent('ldcodex:panel-open',{detail:{open:false,automation:true}}));" : ''}`,
    returnByValue: false,
  }).then((result) => {
...
function automationPanelSetOpen(open) {
  ...
  return cdpSend('Runtime.evaluate', {
    expression: `window.dispatchEvent(new CustomEvent('ldcodex:panel-open',{detail:{open:${!!open},automation:true}}))`,
    returnByValue: false,
...
function startAutomationRun(task, event = null) {
  ...
  const runDeps = { sessionAction, primaryAccount: async () => publicAccounts().find(a=>a.isPrimary) || null, dismissToast: runNotifier.dismiss, completionReport, event, listAccounts: async () => publicAccounts(), currentAccount: publicCurrent, accountSwitch: (account, detail) => withInput(() => automationSwitchAccount(account), !!(detail && detail.restore)), accountStatus: automationAccountStatus, accountCheckin: async (account) => {
    ...
    if (result.ok && !result.skipped && cdp.connected) cdpSend('Runtime.evaluate', { expression: "window.dispatchEvent(new CustomEvent('ldcodex:accounts-updated'))" }).catch(() => {});
    return result;
  ...
  return cdpSend('Runtime.evaluate', { expression: cleanupExpr, returnByValue: false, ...runtimeContext })
    ...
    .then(async (r) => {
      ...
      if (!state || !state.root || !state.widget) {
        ...
        if (reason === 'manual') throw new Error('注入后未检测到 LDCodex 组件（请检查 WorkBuddy 页面是否正常加载）');
      } else {
...
async function exportSessions(ids, password) {
  ...
  const payload = { exportType: 'LDCodex-sessions', version: 1, sessions };
  const content = createEncryptedExport('sessions', payload, password);
  return {
    filename: 'LDCodex-会话导出-' + new Date().toISOString().slice(0, 10) + '.json',
    content,
...
function isAllowedApiOrigin(origin) {
  ...
  try {
    ...
    // LDCodex：新增。本运行时由 LDCodex 的 Tauri 管理界面直接调用，
    // Tauri v2 在 Windows 使用 http://tauri.localhost、在 macOS/Linux 使用 tauri://
    // 作为 webview 来源，均不在上游白名单内，这里显式放行。
    if (host === 'tauri.localhost' || u.protocol === 'tauri:') return true;
...
function hasApiToken(req) {
  const supplied = String(req.headers['x-ldcodex-token'] || '');
...
//  - 开关状态记录在 settings.wbs.noDisturb（LDCodex 自有命名空间，与 CLI 配置互不干扰）。
...
// 原理：将 LDCodex 专属英文指令块追加到 WorkBuddy 全局自定义指令（用户实测「设置-个性化-自定义指令」
...
const AC_TAG_START = '[LDCodex-Rule1 - activate]'; // 区块标识（v2 起改名，避免与其他项目/旧版块冲突）
const AC_TAG_END = '[end LDCodex-Rule1]';
// 兼容清理：新名 Rule1 与旧名 AutoContinue 的块都会被删除（旧版开关残留块升级时一并清除）
const AC_BLOCK_RE = /\[LDCodex-(?:AutoContinue|Rule1)[\s\S]*?\[end LDCodex-(?:AutoContinue|Rule1)\]/g;
...
/**
 * 从 customPrompt 中移除全部 LDCodex-* 指令块（Rule1 新名 + AutoContinue 旧名，含多块），保留用户其他内容。
...
function exportQuickPhrases(ids, password) {
  ...
  const payload = { exportType: 'LDCodex-quick-phrases', version: 1, phrases };
  return {
    filename: 'LDCodex-快捷短语导出-' + new Date().toISOString().slice(0, 10) + '.json',
...
/** 内置资产源目录（首次启动初始化的来源，LDCodex.app 自包含打包）：
 * 1) 脚本同目录 builtin/（app 内置模式：Contents/Resources/scripts/builtin）
 * 2) 项目模式：<项目>/LDCodex.app/Contents/Resources/scripts/builtin
 */
function builtinAssetsDir() {
  const cands = [
    path.join(__dirname, 'builtin'),
    path.join(__dirname, '..', 'LDCodex.app', 'Contents', 'Resources', 'scripts', 'builtin'),
  ];
  if (process.env.WBSWITCH_DIR) {
    cands.push(path.join(process.env.WBSWITCH_DIR, 'LDCodex.app', 'Contents', 'Resources', 'scripts', 'builtin'));
  }
  for (const c of cands) {
    try {
...
/** 内置资产同步：官方壁纸 + LDCodex 主题 + 默认蒙版 10%
...
function initBuiltinAssets() {
  ...
  try {
    ...
    // 2) LDCodex 主题（nebula）→ themes/nebula/（缺失才安装，已有不动）
    ...
    if (fs.existsSync(path.join(thSrc, 'theme.json'))) {
      ...
      if (!fs.existsSync(themeJson)) {
        ...
        log('[init] 已安装 LDCodex 主题（nebula）');
      }
    ...
    // 5) 默认主题 → LDCodex 壁纸主题（仅当 profile 从未设置过主题）
    ...
    if (!fs.existsSync(curFile)) {
      ...
      log('[init] 首次初始化：默认主题 -> LDCodex 壁纸主题（nebula）');
    }
  } catch (e) {
...
const BUILTIN_THEMES = {
  default: { id: 'default', name: '默认（浅色）', author: 'LDCodex', dark: false, colors: {} },
  'oled-dark': {
    id: 'oled-dark', name: 'OLED 纯黑', author: 'LDCodex', dark: true,
    colors: {
  ...
  'eye-care': {
    id: 'eye-care', name: '护眼绿', author: 'LDCodex', dark: false,
    colors: {
      // ---- vscode 主题变量（body 层）----
  ...
  'cyber-purple': {
    id: 'cyber-purple', name: '赛博紫', author: 'LDCodex', dark: true,
    colors: {
      // ---- vscode 主题变量（body 层）----
...
async function applyThemeByCdp(id) {
  ...
  const expr = `(function(){
    ...
    // LDCodex 自定义主题已应用标记：theme-patches 里部分规则用 html[data-wbs-theme] 限定
    // 只在 LDCodex 内置自定义主题下生效（官方默认主题不激活）。
 ...
 * v1 导入：兼容历史固定密码 ldcodex 的导出文件，空密码即走旧格式默认值。
 */
const EXPORT_PASSPHRASE = 'ldcodex';
const EXPORT_KDF_SALT = 'LDCodex-account-export-v1';
...
function handleApi(req, res) {
  ...
  if (req.method === 'OPTIONS') {
    ...
    const headers = {
      ...
      'Access-Control-Allow-Headers': 'Content-Type, X-LDCodex-Token',
  ...
  if (req.method === 'POST' && p === '/api/automations') {
    return readBody(req).then((body) => {
      try {
        ...
        if (index >= 0 && !isSupportedTaskSchema(tasks[index])) throw new Error('任务使用更新的协议，请升级 LDCodex 后再编辑');
    ...
    return readBody(req).then((body) => {
      try {
        ...
        if (action === 'delete') {
          ...
          if (selected.some(task => !isTaskCompatible(task))) throw new Error('任务使用更新的协议，请升级 LDCodex 后再编辑');
  ...
  // LDCodex：客户端探测（WorkBuddy 是否安装 / 可执行文件在哪）。
  // 主题、暂存提示词、快捷短语、免打扰自动点允许、自动化这些能力都依赖 CDP，
  // 需要先知道客户端在不在，界面上才能给客户一句人话，而不是让他自己去猜。
  if (req.method === 'GET' && p === '/api/client-info') {
    let binary = null;
    try { binary = resolveWorkBuddyBinary() || null; } catch (_) { binary = null; }
    let cdpPort = null;
    try {
      cdpPort = (PROFILE && PROFILE.cdp && PROFILE.cdp.port) || (PROFILE && PROFILE.id === 'workbuddy-ai' ? 9223 : 9222);
    } catch (_) { cdpPort = 9222; }
    return json(res, 200, {
      ok: true,
      installed: !!binary,
      binary,
      cdpMode: (PROFILE && PROFILE.cdp && PROFILE.cdp.mode) || 'argument',
      preferredCdpPort: cdpPort,
      cdp: { connected: !!(cdp && cdp.connected), port: (cdp && cdp.port) || null },
    });
  }

  // LDCodex：以 CDP 调试模式重启 WorkBuddy。
  // CDP 只能在 WorkBuddy 启动时用 --remote-debugging-port 打开，事后无法补开，
  // 所以「让增强生效」这一步必须重启客户端。这里复用「无感登录」同一条重启链路
  // （含端口选择、二进制解析、窗口恢复），不再重复实现一份。
  // body: { force?: boolean } —— force=false 只在客户端未运行时直接启动；
  // force=true 会先优雅退出再带 CDP 参数拉起（客户需明确点击确认）。
  if (req.method === 'POST' && p === '/api/relaunch-cdp') {
    return readBody(req).then(async (body) => {
      ...
      try {
        ...
        await relaunchWorkBuddy();
        return json(res, 200, { ok: true, quit, relaunched: true });
      } catch (e) {
        // 原始信息（如 spawnSync powershell ETIMEDOUT）只留给日志，回给界面的是可操作的中文提示。
        log(`[cdp] 以 CDP 模式重启 WorkBuddy 失败: ${e.message}`);
        return json(res, 502, {
          ok: false,
          quit,
          relaunched: false,
          error: '重启 WorkBuddy 没有成功，请手动退出 WorkBuddy 后重试。',
          detail: e.message,
        });
      }
    });
  ...
  if (false) { // LDCodex: 接口已下线
    return readBody(req).then((body) => {
      if (telemetryEnvironmentOverride() !== null) {
        return json(res, 409, { ok: false, error: '诊断设置由 LDCODEX_TELEMETRY 环境变量控制' });
      }
  ...
  // 导入账号：v3/v2 必须输入密码；历史 v1 文件密码可留空（默认 ldcodex）
  if (req.method === 'POST' && p === '/api/accounts/import') {
    return readBody(req).then((body) => {
      try {
        ...
        if (!envelope || envelope.wbsExport !== 'LDCodex') throw new Error('不是 LDCodex 的账号导出文件');
        const enteredPassword = body && typeof body.password === 'string' ? body.password : '';
  ...
  async function probeModelEndpoint(model) {
    ...
    const headers = { Accept: 'application/json, text/plain, */*', 'User-Agent': 'LDCodex probe/1.0' };
...
function startServer() {
  const server = http.createServer((req, res) => {
    ...
    if (req.method === 'GET' && (req.url === '/' || req.url === '/index.html')) {
      ...
      return res.end(
        '<!doctype html><html lang="zh"><meta charset="utf-8"><title>' + LDCODEX_INSTALL_NAME + '</title>' +
        ...
        '<div style="text-align:center"><h1 style="margin:0 0 8px">' + LDCODEX_INSTALL_NAME + ' v' + DAEMON_VERSION + '</h1>' +
...
// 首次启动初始化（新电脑 / 数据目录为空时）：内置壁纸 + LDCodex 主题 + 默认蒙版 10%
initBuiltinAssets();
...
log(`备份目录: ${DATA_DIR}`);
updateDebug('daemon-start', { authFile: currentAuthFile(), dataDir: DATA_DIR, appPath: IS_WIN ? LDCODEX_DIR_WIN : macLDCodexAppPath(), apiPort: UI_PORT_BASE });
