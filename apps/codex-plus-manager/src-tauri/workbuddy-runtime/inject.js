 ...
 *
 * 功能：
 *  1. 右下角圆形黑色悬浮按钮（hover 展开为胶囊显示"LDCodex"）
...
(function () {
  ...
  var WBS_BRAND = WBS_PROFILE_IS_AI ? 'LDCodex' : 'LDCodex';
  ...
  var WBS_LANGUAGE_KEY = 'ldcodex.ui.language';
  var WBS_ACCOUNT_MASK_KEY = 'ldcodex.account.mask.' + PROFILE_ID;
  var WBS_I18N_EN = {
    ...
    '暂存提示词': 'Stash prompt', '暂存': 'Stashed', '引用文本': 'Quote text', '快捷短语': 'Quick phrases', '点击后发送': 'Send on click', '进入插件设置': 'Open plugin settings', '新增': 'Add', '添加': 'Add', '已添加常用语': 'Quick phrase added', '粘贴或输入常用语，Ctrl+Enter 保存': 'Paste or type a phrase, Ctrl+Enter to save', '批量操作': 'Batch actions', '批量管理': 'Batch manage', '全选': 'Select all', '取消全选': 'Deselect all', '已选 0': 'Selected 0',
    ...
    '(无标题)': 'Untitled', 'GitHub 仓库': 'GitHub repository', '去 GitHub Issues 反馈问题': 'Report an issue on GitHub', '在 GitHub 上给 LDCodex 点 Star': 'Star LDCodex on GitHub', 'LDCodex，点击打开面板，可拖动': 'LDCodex – click to open the panel, draggable', '如果 LDCodex 对你有帮助，欢迎在 GitHub 点个 Star。你的支持会让这个小项目持续更新。': 'If LDCodex helps you, please star it on GitHub. Your support keeps this small project alive.',
    ...
    '暂不支持 iframe 内的输入或点击，请使用主页面定位器': 'Input and clicks inside iframes are unsupported; use a main-page locator', 'HTTP 响应超过 1 MiB': 'HTTP response exceeds 1 MiB', '任务不兼容': 'Incompatible task', '任务与当前 LDCodex 不兼容，请升级或使用适配版本': 'This task is incompatible with this LDCodex. Update or use a compatible version.', '排队中': 'Queued', '页面加载': 'Page load', '账号切换后': 'After account switch', '让 WorkBuddy 帮我创建': 'Create with WorkBuddy', '自动化需求': 'Automation requirements', '描述触发时机和需要执行的操作，WorkBuddy 会生成任务并添加到自动化列表。': 'Describe when to trigger and what to do. WorkBuddy will create the task in your automation list.', '例如：每次打开面板时，显示一条欢迎提示。': 'For example: show a welcome notification whenever the panel opens.', '将在新的 WorkBuddy 会话中创建': 'Creates in a new WorkBuddy conversation', '自动化需求必须是 1–6000 字的文本': 'Automation requirements must contain 1–6000 characters',
    ...
    '注入后未检测到 LDCodex 组件（请检查 WorkBuddy 页面是否正常加载）': 'LDCodex not detected after injection (check whether the WorkBuddy page loaded correctly)',
    ...
    '不是 LDCodex 的账号导出文件': 'Not a LDCodex account export file', '导入文件中没有账号数据': 'No account data found in the import file',
    ...
    '诊断设置由 LDCODEX_TELEMETRY 环境变量控制': 'Diagnostics are controlled by the LDCODEX_TELEMETRY environment variable',
  ...
  if (!window.__wbsErrHook) {
    ...
    function wbsReportErr(kind, ev) {
      ...
      if (WBS_DIAGNOSTICS_ENABLED) {
        try {
          fetch(API + '/api/breadcrumb', { method: 'POST', headers: { 'content-type': 'application/json', 'X-LDCodex-Token': WBS_API_TOKEN }, body: JSON.stringify({ msg: 'crash:' + kind + ':' + msg, extra: { stack: (stack || '').slice(0, 1500) } }) }).catch(function () {});
        } catch (_) {}
      }
    }
  ...
  function build() {
    ...
    root.innerHTML = [
      ...
      '<button class="button up" type="button" aria-label="LDCodex，点击打开面板，可拖动">',
      ...
      '<div class="wbs-title-wrap"><div class="wbs-title-row"><div class="wbs-title" id="wbs-title" title="LDCodex">' + WBS_BRAND + '</div>',
    ...
    exploreBtn.innerHTML =
      ...
      // 点击后唤起 LDCodex 面板并定位到「增强」页的快捷短语管理区，所以文案用「进入插件设置」。
      '<a class="wbs-explore-edit" href="#" onclick="return false">进入插件设置</a>' +
      '</span>' +
      '</div>' +
      '</div>' +
      '</div>';
    ...
    function qpDiag(event, extra) {
      ...
      try {
        ...
        fetch(API + '/api/breadcrumb', { method: 'POST', headers: { 'content-type': 'application/json', 'X-LDCodex-Token': WBS_API_TOKEN }, body: JSON.stringify({ msg: '[quick-phrase-diagnostics] ' + JSON.stringify(payload) }) }).catch(function () {});
      } catch (_) {}
    }
    ...
    var MESSAGE_NAV_ENABLED_KEY = 'ldcodex.session.messageNavigationEnabled';
    var SELECTION_QUOTE_ENABLED_KEY = 'ldcodex.session.selectionQuoteEnabled';
    ...
    // 跟随主题设置按钮颜色（放弃跟随官方按钮）：浅色主题=黑底白图标；深色/LDCodex 主题=白底黑图标
    ...
    function crumb(msg) {
      ...
      try {
        fetch(API + '/api/breadcrumb', { method: 'POST', headers: { 'content-type': 'application/json', 'X-LDCodex-Token': WBS_API_TOKEN }, body: JSON.stringify({ msg: msg }) }).catch(function () {});
      } catch (e) {}
    }
      ...
      openSecureTransferModal({
        ...
        onConfirm: function (password, uids) { return api('/api/accounts/export', {
          ...
          body: JSON.stringify({ password: password, uids: uids }),
            ...
            downloadTransfer(r, 'LDCodex-accounts.json');
    ...
    // 导入账号：读文件后输入密码；空密码仅对历史 ldcodex 格式有效。
    ...
    function buildAutomationPane() {
      ...
      function showEditor(task) {
        if (task && task.compatible === false) { toast('任务与当前 LDCodex 不兼容，请升级或使用适配版本', true, root); return; }
        hideEditor();
        var mask = document.createElement('div');
      ...
      function exportAutomationTasks() {
        ...
        api('/api/automations/export', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids: ids }) }).then(function (result) {
          downloadTransfer(result, 'LDCodex-tasks.zip');
      ...
      function importIssueLabel(entry) {
        ...
        if (codes.indexOf('ldcodex_version') >= 0) return '需要更新 LDCodex';
      ...
      if (exportBtn) exportBtn.addEventListener('click', function () {
        ...
        openSecureTransferModal({
          ...
          onConfirm: function (password) { return api('/api/sessions/export', {
            ...
            downloadTransfer(result, 'LDCodex-sessions.json');
    ...
    function wireThemePane() {
      ...
      themePane.addEventListener('click', function (e) {
        ...
        if (segBtn) {
          ...
          // 不做 active 拦截：即使当前已是该主题也强制重新应用（保证「切换到默认主题=强制浅色 / 切换到 LDCodex 主题=强制深色」始终生效，面板状态与真实主题不一致时也能纠正）
    ...
    // daemon 经 CDP dispatch: window.dispatchEvent(new CustomEvent('ldcodex:panel-open',{detail:{open:boolean}}))
    var panelOpenBridge = function (event) {
      var detail = event && event.detail && typeof event.detail === 'object' ? event.detail : {};
      var open = !!detail.open;
      if (open !== state.open) setOpen(open, detail);
    };
    listen(window, 'ldcodex:panel-open', panelOpenBridge);
    registerDisposer(function () { try { window.removeEventListener('ldcodex:panel-open', panelOpenBridge); } catch (_) {} });
    ...
    // ===== 主题系统（LDCodex 换肤）：segmented 切换 =====
    // 主题 = CSS 变量覆盖（--wb-* / --dc-*），daemon 通过 CDP 注入 <style>。
    // 只提供四个入口：官方主题(default) 与 LDCodex 官方主题(nebula)、官方内置深色主题(eye-care/cyber-purple)
          ...
          try {
            ...
            api('/api/theme-image', {
              ...
              return api('/api/theme-save', {
                method: 'POST',
                headers: { 'content-type': 'application/json' },
                body: JSON.stringify({ id: id, name: name || '我的图片皮肤', author: 'LDCodex', dark: dark, image: 'background.webp', colors: colors }),
              }).then(function (sv) {
    ...
    function acSendViaController(gen, controller, assistantId) {
      ...
      try {
        sendPromise = controller.sendPrompt({ blocks: [{ type: 'text', text: AC_CONTINUE_TEXT }] }, {
          _meta: { 'codebuddy.ai': { source: 'ldcodex-auto-continue' } },
        });
      } catch (e) {
      ...
      if (batchExport) batchExport.addEventListener('click', function () {
        ...
        openSecureTransferModal({
          ...
          onConfirm: function (password) { return api('/api/quick-phrases/export', {
            ...
            downloadTransfer(result, 'LDCodex-quick-phrases.json');
    ...
    listen(window, 'ldcodex:accounts-updated', function () {
...
'.wbs-ac-log{margin:0;padding:6px 8px;max-height:180px;overflow:auto;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:11px;line-height:1.55;color:var(--wb-icon-secondary,#555);white-space:pre-wrap;word-break:break-all}',
    ...
    // 文案是「进入插件设置」（比原来的「编辑 →」长），nowrap 防止中文在面板里被折成两行
    '.wbs-explore-edit{font-size:12px;font-weight:500;color:var(--wb-icon-secondary,#666);text-decoration:none;cursor:pointer;white-space:nowrap}',
    '.wbs-explore-edit:hover{color:var(--wb-color-text-primary,#1f1f1f)}',
    /* 底部第 1 行：左侧「+ 添加」按钮，右侧是发送提示与「进入插件设置」 */
    ...
    /* 主题切换 segmented（默认 / LDCodex / 官方内置深色主题）：样式与下方壁纸来源切换按钮统一 */
    ...
    /* 选择登录方式：遮罩限定在 LDCodex 面板内部，沿用会话弹窗的层级与按钮风格 */
