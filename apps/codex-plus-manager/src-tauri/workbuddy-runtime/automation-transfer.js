...
function exportTasks(tasks, ids) {
  ...
  if (entries.length === 1) return { count: 1, filename: 'LDCodex-task-' + entries[0].name, encoding: 'utf8', content: entries[0].content.toString('utf8'), mimeType: 'application/json' };
  return { count: entries.length, filename: 'LDCodex-tasks-' + new Date().toISOString().slice(0, 10) + '.zip', encoding: 'base64', content: encodeZip(entries).toString('base64'), mimeType: 'application/zip' };
}
  ...
  const entries = readFiles(payload).map((file, index) => {
    ...
    try {
      ...
      if (preview.kind === 'package') {
        ...
        task['x-ldcodex-import'] = { kind: 'local-file', packageId: preview.package.id, packageVersion: preview.package.version, sha256 };
