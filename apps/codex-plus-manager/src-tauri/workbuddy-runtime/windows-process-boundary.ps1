...
function ConvertTo-LDCodexProcessRecord {
...
function ConvertTo-LDCodexProcessRecordIfPresent {
  param([Parameter(Mandatory = $true)]$Process)
  try { return ConvertTo-LDCodexProcessRecord -Process $Process }
  catch {
    if (Test-CimProcessDisappearedError $_) { return $null }
    throw
  }
...
function Get-StrictProcessRecord {
  ...
  $record = ConvertTo-LDCodexProcessRecord -Process $rows[0]
...
function Get-AuthenticatedLDCodexStatus {
  param(
  ...
  try {
    $response = Invoke-WebRequest `
      ...
      -Headers @{ 'X-LDCodex-Token' = $token } `
      -TimeoutSec 3 `
      -ErrorAction Stop
    $status = $response.Content | ConvertFrom-Json -ErrorAction Stop
  } catch {
    throw ('无法通过本地身份凭证确认正在运行的 LDCodex: ' + $_.Exception.Message)
  }
  $statusPid = 0
  if ($null -eq $status -or $status.ok -ne $true -or
      -not [int]::TryParse([string]$status.pid, [ref]$statusPid) -or $statusPid -le 0) {
    throw '本地 LDCodex 状态中的 PID 无效'
  }
  if ([string]$status.profile.id -cne $ExpectedProfile) { throw '本地 LDCodex profile 不匹配' }
  if (-not $AllowVersionMismatch -and [string]$status.version -cne $ExpectedVersion) {
    throw '本地 LDCodex version 不匹配'
  }
  if ([string]$status.privilege -cne 'elevated') { throw '本地 LDCodex 不是管理员权限进程' }
  if ([string]::IsNullOrWhiteSpace([string]$status.dataDir)) {
    throw '本地 LDCodex 未接受当前 profile 身份凭证'
  }
...
function Stop-VerifiedLDCodexLifecycle {
  param(
    [Parameter(Mandatory = $true)][string]$DataDir,
    [Parameter(Mandatory = $true)][int]$Port,
  ...
  # 所有候选先完成身份验证；外来监听进程不会导致任何 LDCodex 进程被提前终止。
  if ($null -ne $daemon) {
