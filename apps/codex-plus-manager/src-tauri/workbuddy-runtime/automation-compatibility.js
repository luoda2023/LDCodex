...
function validateRequirements(requires) {
  ...
  const allowed=new Set(['minLDCodexVersion','taskSchemaVersion','capabilities','profiles','platforms']);
  ...
  stableVersion(requires.minLDCodexVersion);
...
function assessRequirements(requires, runtime, actualCapabilities=[]) {
  ...
  if(requires){
    ...
    if(!runtime.version||!versionAtLeast(runtime.version,requires.minLDCodexVersion))issues.push({code:'ldcodex_version',required:requires.minLDCodexVersion});
