...
const AGENT_EXAMPLES = [
  {
    ...
    promptZh: '每次切换账号或 WorkBuddy 页面加载完成后，立即从窗口底部弹出 toast，展示当前账号的名称、剩余积分和连续活跃天数。同一次加载只提示一次，快速切换时不要显示上一个账号的过时结果。积分不限量时显示“不限量”，天数查询失败时显示“暂不可用”。把这个任务添加到 LDCodex 的自动化列表。',
    promptEn: 'After each account switch or WorkBuddy page load, immediately show a toast at the window bottom with the current account name, remaining credits and consecutive active days. Show it once per load, and discard stale results when accounts change quickly. Display Unlimited for unlimited credits, and Unavailable if active days cannot be retrieved. Add this task to LDCodex Automation.',
...
const CAPABILITIES = [
  ...
  { id: 'event.panelOpened', zh: '打开面板触发', en: 'Panel opened', descriptionZh: '用户打开 LDCodex 面板时触发；自动化恢复面板不会触发。', descriptionEn: 'Run when the user opens the panel, excluding automatic panel restoration.', example: { trigger: { type: 'panelOpened' } } },
...
function capabilityText(language = 'zh') {
  ...
  const lines = zh ? [
    ...
    'LDCodex Automation Task Protocol v1',
    '',
    'Task JSON:',
...
function agentGuideText(paths, profileId) {
  return [
    'LDCodex automation agent bridge',
    '',
...
function createAgentRequest(dataDir, options = {}) {
  ...
  const instructions = language === 'en' ? [
    'LDCodex has prepared a restricted local bridge for this request.',
    ...
    '5. Wait for LDCodex to create this result file and read it: ' + resultFile,
    ...
    '5. 等待 LDCodex 生成并读取结果文件：' + resultFile,
...
async function executeTask(taskInput, options = {}) {
  ...
  if (!isTaskCompatible(task)) throw new Error('任务与当前 LDCodex 不兼容，请升级或使用适配版本');
