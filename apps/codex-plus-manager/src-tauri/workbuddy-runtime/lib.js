...
const LEGACY_DATA_DIR = IS_WIN
  ? null
  : path.join(os.homedir(), 'Library', 'Application Support', 'LDLegacy');
...
function defaultDataDir() {
  // 旧版 launchd 可能把 WBSWITCH_DATA_DIR 设成 LDLegacy；新版本始终落到 LDCodex，
...
/**
 * 兼容旧版账号备份：把 LDLegacy/accounts 中尚未存在于 LDCodex 的账号复制过来。
 * 只对平台默认 LDCodex 目录执行，显式自定义数据目录不做隐式迁移。
 * 源目录和文件均保留，重复调用幂等。
 */
...
function deleteAccount(dataDir, uid, log = () => {}) {
  ...
  // 旧版 LDLegacy 目录仍会在每次启动时迁移缺失的账号备份。删除新目录
