...
function assertStandardWindowsPrivilege(run = spawnSync) {
  ...
  if (privilege !== 'standard') throw new Error('拒绝以管理员或 elevated 权限运行 LDCodex');
  return privilege;
}
 ...
 * LDCodex：容错版权限探测。
 *
 * 上游实现会在 PowerShell 不可用 / 超时 / 被组策略拦截时直接抛错，而 daemon 是在
 * require 阶段调用它的 —— 一旦抛错，整个增强服务都起不来（客户只会看到"未运行"，
 * 完全不知道发生了什么）。这里改成永不抛错：探测不出来就返回 'unknown'，
 * 由调用方按「普通权限」保守处理。
 */
function detectWindowsPrivilegeSafe(run = spawnSync) {
  try {
    return detectWindowsPrivilege(run);
  } catch (_) {
    return 'unknown';
  }
}
...
function buildNativeProcessQuery(helperPath, processSource) {
  ...
  return `. '${quotedHelper}'; ${source} | ForEach-Object { ConvertTo-LDCodexProcessRecordIfPresent -Process $_ } | Where-Object { $null -ne $_ } | ConvertTo-Json -Compress -Depth 4`;
}
