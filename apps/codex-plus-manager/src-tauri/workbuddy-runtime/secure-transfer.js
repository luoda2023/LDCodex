...
function createEncryptedExport(kind, payload, password, createdAt) {
  ...
  if (!payload || typeof payload !== 'object' || payload.exportType !== 'LDCodex-' + normalizedKind) {
...
function openEncryptedExport(content, expectedKind, password) {
  ...
  if (!envelope || envelope.wbsExport !== 'LDCodex' || ![2, EXPORT_VERSION].includes(Number(envelope.version))) {
    throw new Error('不是有效的 LDCodex 加密导出文件');
  }
  ...
  if (!payload || payload.exportType !== 'LDCodex-' + normalizedKind) throw new Error('导出文件类型不匹配');
  return payload;
}
