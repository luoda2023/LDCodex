...
module.exports = [
  ...
  {
    id: 'patch-92',
    desc: '侧栏会话卡片未读圆点(_unreadDot_，挂 cb-agent-card 的 header 内)适配主题：圆点由 ::before 实现（官方硬编码 rgb(0,194,154) 青绿、局部 --wb-accent-unread 也被官方污染为青绿；无 --wb-accent 定义导致 fallback 蓝色突兀），改为主题文字色 var(--wb-color-text-primary)（LDCodex 建主题必定义：深色=白/浅色=深绿，与主题绝对协调），fallback #8a8f98 中性灰；容器自身保持透明背景（用户 08-30 10:40 修正 10:36 的蓝色）',
