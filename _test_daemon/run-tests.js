...
(async () => {
  ...
  // ── T21 安装器钩子不得把「安装程序 / 卸载程序自己」杀掉（2026-09-13 连撞两次真实事故）──
  // 【血案 1】安装器把自己杀了。
  //   PowerShell 兜底用 `-like '*LDCodex*'` 匹配要结束的进程，
  ...
  // 【血案 2】卸载器把「正在等它的安装器」杀了。
  //   第一版只加 `-and $_.ExecutablePath -ne '$EXEPATH'`，但 **$EXEPATH 只等于当前这个
  //   exe 自己** —— 在卸载器里它 = uninstall.exe 的路径，**保护不到父进程安装器**。
  //   installer.nsi 的 reinst_uninstall 分支是 `ExecWait '$R1'` 同步等卸载器跑完，
  //   卸载器一启动就执行本宏 → 把还活着的 LDCodex_*_x64-setup.exe 杀掉
  //   → 安装器 ExitCode -1、7 秒退出，而且旧版已经被删干净（主程序没了、注册表键也没了）。
  // ✅ 正确修法是**按进程名排除**：`*setup*.exe` 覆盖安装器（无论它是自己还是父进程）、
  //   `*uninstall*.exe` 覆盖卸载器自己（它的路径 C:\Program Files\LDCodex\uninstall.exe
  //   同样含 LDCodex）。$EXEPATH 那条保留作双保险。
  //   设计取向：漏杀只是「文件被占用」这种看得见的报错，误杀是「静默退出 -1」，
  //   所以宁可漏杀 —— 这也是用 -notlike 排除、而不是把过滤器收得更紧的原因。
  const psKillLine = hooksCode.split(/\r?\n/).find((l) => /Get-CimInstance Win32_Process/.test(l)) || '';
  // ⚠️ 用 includes 而不是正则：这行里 $ \ ' * 全是要转义的元字符，正则极易写错还不报错。
  rec('T21a PowerShell 兜底杀进程必须在 *LDCodex* 之外排除安装器与卸载器（进程名 + $EXEPATH 双保险）',
    psKillLine.includes("*LDCodex*") &&
      psKillLine.includes("*setup*.exe") &&
      psKillLine.includes("*uninstall*.exe") &&
      psKillLine.includes("$EXEPATH") &&
      psKillLine.includes("-ne") &&
      psKillLine.includes("-notlike"),
    '否则安装器会把自己 / 卸载器会把父安装器 Stop-Process 掉 → 静默退出 -1，什么都装不上');
  rec('T21b 排除条件必须紧贴 *LDCodex* 那个判断（不是写在别处糊弄）',
    psKillLine.includes(
      "$\\'*LDCodex*$\\' -and $$_.Name -notlike $\\'*setup*.exe$\\' -and $$_.Name -notlike $\\'*uninstall*.exe$\\' -and $$_.ExecutablePath -ne $\\'$EXEPATH$\\'"
    ),
  ...
  rec('T21d 必须同时排除「安装器」与「卸载器」两类进程名（即至少两个 -notlike）',
