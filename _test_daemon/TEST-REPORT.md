...
| 测试集 | 结果 |
|---|---|
| 功能接口测试（`run-tests.js`） | **179 / 179 全部通过** ✅（88.8.6 新增 T25 系列 23 项：更新源格式 / minisign 公钥 / 安装模式 / 三命令拆分 / 前后端事件名一致 / 发版流水线 / 更新界面 i18n 词条齐全；**新增 T26 系列 6 项：WorkBuddy 对话管理**；**新增 T20f：安装核验脚本必须直接验主程序本体**；含 88.8.5 的 T23 系列 7 项 + T24 系列 8 项） |
...
| 本机安装版本核验（`verify-installed-version.mjs`） | **8 / 8 ✅ 全部通过**（88.8.6 已装上；⚠️ 装之前会报 4/7，因为安装目录里还是上一版，见「一之零K」。**第 8 项是 88.8.6 补的：前面 7 项只验 `workbuddy-runtime`，不改 daemon 就永远全绿，会给出假阳性** —— 见「一之零L」） |
...
## 一之零L、🔴 安装核验的假阳性：7/7 全绿，但装的还是旧主程序（2026-09-15）

### 现象

出完 88.8.6 的安装包、准备装之前，例行跑了 `verify-installed-version.mjs` ——
**7/7 全绿**。但已装的 `LDCodexManager.exe`（47,512,576 B / 09-15 16:32）里：

| 关键字 | 命中 |
|---|---|
| `list_workbuddy_sessions` | 0 |
| `workbuddy-sessions-*` | 0 |
| 「彻底清除」 | 0 |

新功能一个都没在里面，版本号却是 88.8.6。

### 真因

这个脚本比的是 **`workbuddy-runtime/`（daemon 那 38 个文件）** + `inject.js` + 注册表版本号。
本次新功能改的是 `App.tsx` 和 `commands.rs`，**一个都不在它的校验范围里** ——
只要这轮没动 daemon（绝大多数前端 / 后端改动都不动），它**永远全绿**。

> 版本号是 88.8.6 也不矛盾：那一版是上一轮「接入自动升级」时装的，
> daemon 的版本常量已经 bump 过，所以连 `DAEMON_VERSION` 都对得上。
> **版本号对 ≠ 内容是这轮构建的**（与记忆里那条「判定安装包是否含最新代码别只看版本号」是同一类坑）。

### 修法：加第 8 项，直接验主程序本体

用 **dist 资源文件名指纹**：Tauri 把 `dist` 打进 exe，资源**清单是明文、内容是压缩包**。

- ✅ 能搜到：`index-CFls5wWC.js`（文件名，hash 随前端内容变化 → 天然版本指纹）
- ❌ 搜不到：JS 里的类名 / 中文文案（压缩包）
  —— ⚠️ 所以 exe 里能搜到 `purge_workbuddy_sessions`、`彻底清除`，那其实是 **Rust 侧字面量**，
  **不能**拿来当「前端已更新」的证据。

配 exe 大小与新构建产物一致（47,662,080 B）即足够。加完：装之前确实会红，装上后 8/8。

### 顺带两个坑

1. **守卫别用正则比带反斜杠的字面量**。`index-[A-Za-z0-9_-]+\.js$` 写成正则要层层转义，
   第一次写成「看着对但永不命中」，白查十分钟。改成 `String.includes()` 直接比字面量。
   （`T20f` 现在就是这样写的。）
2. **在 bash 里用 `node -e "..."` 验证含反斜杠的正则会被 shell 吃掉转义**，
   得到假的 MISS/PASS。要验证就写成 `.mjs` 文件再跑。

### 这一轮的完整结果（2026-09-15 21:10，装上 88.8.6 后）

```
account-usage 16/16 ｜ make-latest-json 11/11 ｜ 更新签名校验 4/4
安装版本核验 8/8（含新加的主程序指纹项）
run-tests.js 179/179（T26 系列 6 项 + T20f 全通过）
no-disturb 16/16 ｜ 布局实测 14/14 ｜ 插件面板实测 60/60 ｜ .fill 实测 30/30
test-run.sh 退出码 0，8m02s
```

> 上一轮（9 月 15 日下午）`test-run.sh` 卡在布局组之后、插件面板组起不来无头 Chrome，
> 这次一次跑完 —— C 盘从 2.9G(96%) 回升到 5.4G(91%) 后恢复正常。
...
| 版本 | 算法 | 判定 |
|---|---|---|
| 第一版 | 本地自然日，生效点 = 明天 `00:00` | ❌ 被用户否决：「不是每天0点」 |
| 第二版 | **最后一次切换的时刻 + 24 小时**（滚动窗口） | ✅ 采纳 |

`account-usage.js` 新增 `RESET_WINDOW_MS = 24*60*60*1000` 与 `nextEffectiveAt(lastSwitchAt, now)`；
起点取 `history` **最后一条**的 `at`。`/api/account/usage/summary` 下发
`lastSwitchAt` / `nextEffectiveAt` / `ready` / `fromUid` / `toUid`。
没有切换记录时 `lastSwitchAt = null`，前端不显示这张卡。

### 顺带修掉的隐蔽 bug：「今日切换 N 次」恒为 0

`account-usage.js` 的 `localDay()` 曾拼成 **`"2026-0913"`**（漏了月与日之间的 `-`）。
自写自读格式自洽、看不出问题，但 daemon 是 `store.all(todayStr())` 调用 ——
`"2026-0913" !== "2026-09-13"` → `today` 恒为 0。
已改成与 daemon `todayStr()` **逐字符一致**，并加了一条直接抠 `daemon.js` 里
`todayStr` 源码来比对的测试，防止两边再各自漂移。

### 插件「关于」页

删除「发送错误诊断」卡（连带「推荐开启」）与「会话监听日志」卡；新增 `wbs-about-meta`
元数据行（插件版本 / 注入方式）；面板左上角版本行只显示 `v88.8.5`；
删除已无调用方的 `wbsClientVersion()`。三处引用点均判空，安全。

### 回归守卫

| 系列 | 内容 |
|---|---|
| T23（7 项） | summary 端点下发 nextEffectiveAt；**禁止 `setHours(24)` / `getDate()+1` 日历日写法**；RESET_WINDOW_MS 必须是 24h；`nextEffectiveAt`/`RESET_WINDOW_MS` 必须导出；前端用 nextEffectiveAt 且不得再出现 nextResetAt；前端必须显示「上次切换」时间；localDay 与 todayStr 同格式 |
| T24（6 项） | 关于页删除「发送错误诊断 / 推荐开启 / 会话监听日志」；改用 `wbs-about-meta` 排版；不再有 `wbsClientVersion`；版本号必须过滤 `__WBS_VERSION__` 占位符 |

⚠️ T24b/c 的断言范围限定在 `buildAboutPane()` 区块内 —— `inject.js` 的 i18n 词典仍保留
「发送错误诊断 / 推荐开启」词条（英文映射表，不是页面结构），全文件搜会一直命中。
同理 T23b / T24e 是**否定断言，必须先剥注释** —— 源码注释里特意写了被禁用的写法与标识符。

### 结果

`run-tests.js` **146/146**、`account-usage.test.js` **16/16**、no-disturb 16/16、
...
## 一之零J、88.8.5 复跑：`test-run.sh` 干净退出结案 + 源码/安装包一致性重编（2026-09-14）

### ① 「脚本跑完不返回」结案：**不是 bug**

前几轮我反复看到「测试全绿了但脚本不返回」，怀疑无头 Chrome 残留进程吊住管道。
本轮显式记录时间戳 + 退出码跑完整回归，**结案**：

```
START 2026-09-14 10:09:53  →  TESTRUN_EXIT=0  →  END 10:13:37
START 2026-09-14 10:28:16  →  TESTRUN_EXIT=0  →  END 10:30:26
```

两次都**自然退出、退出码 0**（3m44s / 2m12s），7 组全绿。

**为什么以前看着像挂住**：
1. `test-run.sh` 末尾**只有 `kill` + `exit`，没有汇总 echo** —— 日志停在最后一组汇总行
   （`.fill 30/30`）就是**正常终态**。以前把「没有后续输出」误当成「没结束」。
2. TaskOutput 默认超时 10 分钟；磁盘 100% 满那几轮会拖到 10 分钟以上。
3. 后台任务句柄丢失（`Background task "8V1OGr" not found`）加剧了错觉 ——
   **可靠证据是日志文件，不是任务句柄**。

### ② ⚠️ 源码里又落盘了一处清理 → 源码与安装包不一致 → 重编重装

02:18 出包**之后**，`inject.js` 里又落盘了一处「关于」页重整的收尾清理：

| 改动 | 为什么安全 |
|---|---|
| 删 `aboutPane.querySelector('#wbs-monitor-log-card')` 那两行 | 那张卡已按用户要求删除，元素不存在 → `querySelector` 返回 `null`，本来就被 `if (monitorLogCard)` 兜住，**零行为变化** |
| 删 `var hiddenToolsUnlocked` 声明 + 唯一一次 `= true` 赋值 | `grep` 全文件确认**没有任何读取处**，纯只写不读 |

**后果**：`verify-installed-version.mjs` 的「安装目录与源码逐文件一致 38/38」变成**假绿**
（核验 10:09:56 通过，文件 10:12:04 才变）。这不是功能问题，但会掩盖真正的漂移。

**处置**（选重编而不是回退，保住这处清理）：

| 步骤 | 结果 |
|---|---|
| ① 构建前记 `inject.js` sha256 | `51089f8f…` |
| ② `npm run build` | ✅ `BUILD_EXIT=0`，**10m15s** |
| ③ 构建后再记 sha256 | `51089f8f…` —— **一致，无中途落盘** |
| ④ `/S /UPDATE` 静默安装 | ✅ `INSTALL_EXIT=0`，**139 秒** |
| ⑤ `verify-installed-version.mjs` | ✅ **7/7**，含「38 一致 / 0 不一致 / 0 缺失」 |
| ⑥ 源码 vs 安装目录 `inject.js` sha256 | ✅ **完全相同**（`51089f8f…`） |
| ⑦ 全量回归 | ✅ `TESTRUN_EXIT=0`，2m12s，7 组全绿 |

**教训**：判断「删掉是否安全」不能只看那几行，要 `grep` 整个文件的**读取处**；
一致性核验要**在构建前后各记一次 sha256**，否则中途落盘的改动会让「逐字节一致」变假绿。

### ③ 现行产物

`target/release/bundle/nsis/LDCodex_88.8.5_x64-setup.exe`
**53,260,017 字节 / 2026-09-14 10:31:04 / SHA-256 `039B141E…`**

> 88.8.5 共出过三版包（01:09 改语义前 / 02:18 漏编清理 / **10:31 现行**），只有 10:31 有效。

**拆包核对**（`_tmp/7zfull/7z.exe x`，最硬的判据）：

| 项 | 结果 |
|---|---|
| 包内 `workbuddy-runtime\`（56 个文件） vs 源码 `diff -r` | ✅ **无差异** |
| 包内 `inject.js` sha256 vs 源码 | ✅ 相同（`51089f8f…`，808,177 字节） |
| 包内 `LDCodexManager.exe` vs **已安装的** | ✅ 逐字节相同（`977fc585…`，46,025,728 字节） |

> ⚠️ **一个观察**：产物文件在构建任务「已完成」（10:25:06）之后、**10:31:04 又被写了一次**
> （大小 53,268,017 → 53,260,017、sha256 `06FE0D76…` → `039B141E…`）。
> 已确认改写后的包**内容与源码一致**（上表），且此后 20 秒 + 多轮检查哈希稳定。
> 现象与本轮 `inject.js` 的「延迟落盘」同类 —— **凡是拿产物哈希/大小做对账，都要在写入稳定后再取一次**。
...
### 回归守卫：T21 系列（3 项）→ 见下方「一之零G」，已扩成 4 项
...
## 一之零G、🔴 第二个自杀路径：**卸载器把「正在等它的安装器」杀了**（2026-09-13 23:20 修复）

修完「一之零F」之后 `/S /UPDATE` 已经能装（15~18 秒、ExitCode 0），
但换成 `/P`（＝模拟用户一路点下一步）**又是 6.8 秒 ExitCode -1**，
而且**旧版本已经被删掉了** —— 主程序没了、注册表键也没了，现场比 F 更糟。

### 现象

```
LDCodex_88.8.4_x64-setup.exe /S /UPDATE  → 15~18 秒装完，ExitCode 0  ✅
LDCodex_88.8.4_x64-setup.exe /P          → 6.8 秒退出，ExitCode -1    ❌
  且 C:\Program Files\LDCodex 里 LDCodexManager.exe 已被删除、
  HKLM\...\Uninstall\LDCodex 注册表键也没了（＝卸载跑完了，安装没跑完）
```

### 根因：两层叠加

**第一层（Tauri 官方模板缺陷）** —— `PageReinstall` 在 `$PassiveMode = 1` 时**不创建单选按钮**，
直接 `Call PageLeaveReinstall`；而 `PageLeaveReinstall` 首句是 `${NSD_GetState} $R2 $R1`，
`$R2` 这时还只是上一句赋的**文案字符串**（`添加/重新安装组件`），不是窗口句柄 → 返回 `0`
→ 同版本分支 `$R0 = 0 / $R1 = 0` → 走 `${Else}` = **`Goto reinst_uninstall`**。
**被动模式的默认行为与交互模式相反**（交互模式默认选中第一项 = 不卸载）。

**第二层（我们的 bug）** —— `reinst_uninstall` 是 `ExecWait '$R1' $0`：安装器**同步等**卸载器跑完。
卸载器一启动就执行 `NSIS_HOOK_PREUNINSTALL` → `LDCodexKillOnce`。
而 F 那版的修法是 `$_.ExecutablePath -ne '$EXEPATH'` —— **`$EXEPATH` 只等于「当前这个 exe 自己」**，
在卸载器里它是 `C:\Program Files\LDCodex\uninstall.exe`，**保护不到父进程安装器**。
于是卸载器把还活着的 `LDCodex_*_x64-setup.exe` 一起 `Stop-Process -Force` 了。

### 定位：打点日志实锤

在 `installer.nsi` 的 8 个关键点插 `FileOpen/FileWrite`（脚本：`_tmp/make-debug-nsi2.js`），
`makensis` 重编后跑 `/P`，日志**止步于 `before-ExecWait`**：
...
[before-ExecWait] 即将执行卸载命令: "C:\Program Files\LDCodex\uninstall.exe" /P _?=C:\Program Files\LDCodex
   ← after-ExecWait 再也没写出来
```

（⚠️ 本机 NSIS 的 `FileOpen … "a"` 不截断，日志有残尾，要按「最后出现在文件**开头**的打点」判断进度。）

### 一个必须注意的坑

**改完 hooks.nsh 重编包还不够 —— 必须先用新版装一次，把安装目录里的 `uninstall.exe` 也换成新版。**
`reinst_uninstall` 调用的是**上一次装进去的那个** `C:\Program Files\LDCodex\uninstall.exe`，不是包里的新文件。
23:07 重编了包但 `/P` 仍 -1，就是因为目录里的卸载器还是 23:02 装的旧版。
...
设计取向：**漏杀只是「文件被占用」这种看得见的报错；误杀是「静默退出 -1」** ——
所以宁可漏杀，用 `-notlike` 排除，而不是把 `*LDCodex*` 收得更紧。

### 修复后实测

| 项目 | 结果 |
|---|---|
| 打点版 `/P`（OutFile 刻意用正式命名 `LDCodex_88.8.4_x64-setup-log.exe`） | **22.5 秒装完，ExitCode 0** ✅ |
| 打点日志 | 走完 `reinst_uninstall` → `ExecWait` → `post-PREINSTALL`（没被自己杀掉）→ `MainBinary-written` ✅ |
| 对照：把 OutFile 改成 `nsis-log2.exe`（名字不含 setup） | **ExitCode -1**，日志止步 `before-ExecWait` —— 反向证明「按进程名排除」确是生效的那条规则 |
| 正式包 `/S /UPDATE` 重装 | **16.3 秒，ExitCode 0** ✅ |
| `verify-installed-version.mjs` | **7 / 7 ✅**（38 个运行时文件逐字节一致，`DisplayVersion = 88.8.4`） |
| `run-tests.js` | **127 / 127 ✅**（126 + 新增 T21d） |

### 回归守卫：T21 扩成 4 项

| # | 断言 |
|---|---|
| T21a | 必须同时含 `*LDCodex*` + `*setup*.exe` + `*uninstall*.exe` + `$EXEPATH` + `-ne` + `-notlike` |
| T21b | 排除条件必须**紧贴** `*LDCodex*`，用 `includes` 校验确切形态（**不用正则**） |
| T21c | `*workbuddy-runtime*` 仍在（不能因为修事故就顺手删掉） |
| T21d | **至少两个 `-notlike`** —— 只排一个不够：卸载器里的 `$EXEPATH` 保护不到父安装器 |

### 教训

- **`$EXEPATH` 只保护「自己」，保护不了「调用链上的父进程」。** 凡是「A 同步等 B、B 里又杀进程」的结构，
  都必须按**进程名**（或 PID 族）排除，不能只排自己。
- **改了卸载/杀进程逻辑后，要确认生效的到底是哪一份二进制** —— 安装器调的是**装在系统里那个**卸载器，
  不是你刚编出来的那个。
...
`npm run build`（**5m14s、`BUILD_EXIT=0`**），用标准流程产出的包重验一遍：

```
产物  target/release/bundle/nsis/LDCodex_88.8.4_x64-setup.exe
      53,266,693 字节 / 2026-09-13 23:31:19
      SHA256 6895340C99470BA1C96531FA12C43C09A656DCEDF9B281107786CEB16AF89A56

拆包直证（7z.exe 解出包内 LDCodexManager.exe）：
      含 "LDCodex 管理工具 v"        ✅（88.8.4 新增的 tooltip 文案）
      含 "左键显示窗口，右键打开菜单"  ✅
      含 "{version}"                 ✅（版本号占位符，运行时用 CARGO_PKG_VERSION 填）
      精确匹配 "88.8.4" 8 处          ✅
      精确匹配 "88.8.1" / "88.8.3"   ✅ 各 0 次
      ⚠️ grep -c 会误报（88.8.1 命中 1、88.8.3 命中 2），必须用 grep -o "88\.8\.1" 精确匹配

实装  /S /UPDATE → 14.4 秒，ExitCode 0，DisplayVersion = 88.8.4
      uninstall.exe mtime 23:32:24（正式构建版，两个自杀修复都在里面）
核验  verify-installed-version.mjs → 7/7 ✅（38 个运行时文件逐字节一致）
```
...
- 源码级另有 **T18 系列 5 项**（`.wbs-panel` 必须受视口约束 / `.wbs-body` 不得有估值 / JS 内联 `maxHeight` 不得钉 650px / 三处滚动条不得透明）、**T19 系列 6 项**（安装器钩子）与 **T20 系列 5 项**（安装版本核验脚本：必须存在 / 必须被 `test-run.sh` 调用 / **必须保持只读** / 读注册表必须带降级 / 不得用 `toISOString()` 打时间）静态守卫。
...
## 一之零D-补二、新增「本机安装版本核验」（`verify-installed-version.mjs`）

### 为什么加它

出包之后用户问「**你怎么回事？你没有构建出来新版本吗还是原来的那一版**」。
排查中发现两件事必须固化下来，否则下次还会踩：

1. **安装包哈希不能用来判断新旧** —— 我一度以为「用当前源码重新 makensis 编译，比对 sha256」
   就能证明包是新构建的，结果哈希对不上（差 669 字节）就误判成「NSIS 产物不可复现（时间戳）」。
   做了对照实验才推翻：**同一份 `installer.nsi` 连编两次产物哈希完全相同**；
   换 Tauri 用的 `-V3` 再编一次也和 `-V2` 一模一样（`tauri-bundler-2.9.4/…/nsis/mod.rs:695-700`
...
`_test_daemon/verify-installed-version.mjs`（只读：不改文件、不写注册表、不起进程）：

| # | 断言 | 依据 |
|---|---|---|
| 1 | 读到源码权威版本号 | `Cargo.toml` 的 `[workspace.package] version` |
| 2 | 源码 `daemon.js` 的 `DAEMON_VERSION` 与之一致 | T14 系列也在守这条，这里顺带复核 |
| 3 | 安装目录存在 | `C:\Program Files\LDCodex`（可用 `LDCODEX_INSTALL_DIR` 覆盖） |
| 4 | **安装目录 `workbuddy-runtime\` 与源码逐文件 sha256 一致** | 源码是唯一权威；全等即说明运行时吃到了当前源码 |
| 5 | 安装目录 `daemon.js` 的 `DAEMON_VERSION` = 期望版本 | 正则抠源码字面量 |
| 6 | 注册表 `DisplayVersion` = 期望版本 | `reg query` → 失败降级 PowerShell `Get-ItemProperty` |
| 7 | 安装的 `inject.js` 含 88.8.3 面板高度修复标记 `100vh - 44px` | 源码里 5 处，安装侧必须同样 5 处 |

用法：`node _test_daemon/verify-installed-version.mjs`（诊断，永远 exit 0）；
加 `--strict` 则不匹配 exit 1。已接进 `test-run.sh` 开头做**信息性**步骤（`|| true`，不参与成败判定）。

### 首次运行结果（2026-09-13，用户尚未装上 88.8.3）

```
3/7 ❌ 失败 4
FAIL 安装目录 workbuddy-runtime 与源码逐文件一致（共 38 个） :: 35 一致 / 3 不一致 / 0 缺失
     不一致：daemon.js, inject.js, package.json
FAIL 安装目录 daemon.js 的 DAEMON_VERSION = 期望版本  ::  安装 = 88.8.1 / 期望 = 88.8.3
FAIL 注册表 DisplayVersion = 期望版本  ::  DisplayVersion = 88.8.1
FAIL 安装的 inject.js 含 88.8.3 面板高度修复标记「100vh - 44px」  ::  安装命中 0 处 / 源码 5 处
```

38 个文件里 **35 个哈希一致** —— 说明 `88.8.1 → 88.8.3` 只动了 3 个文件
（`daemon.js` / `inject.js` / `package.json`），其余原样。安装目录 mtime `17:15:26`、
`uninstall.exe` mtime `17:15:38`，**全部早于出包时间 19:12:50** → 结论：**从未装上 88.8.3**。

### 踩到的两个坑（都已在脚本里修掉）

1. ⚠️ **`reg.exe` 在本机沙箱程序黑名单里** —— Bash 里报 `PROGRAM BLOCKED BY SECURITY POLICY`，
   **从 Node 里调只得到 `spawnSync reg EPERM`**（不带黑名单文案，光按文案匹配会漏判）。
   若把它当成「键不存在」→ 会把「读不到」**误报成版本不对**。
   修法：`reg query` 失败（非 `status===1`）就降级到 PowerShell `Get-ItemProperty`
   （实测可行，读到了 `DisplayVersion = 88.8.1`）；两条路都被拦才记 **SKIP 而不是 FAIL**。
2. ⚠️ **别用 `toISOString()` 打时间** —— 那是 UTC，比北京时间少 8 小时，
   实测把 `17:15:26` 打成 `09:15:26`，看着像旧文件。要手工拼本地时间。

### 接入 `test-run.sh` + 新增 T20 系列守卫

`test-run.sh` 开头加了一步 `"$NODE" "$TMP/verify-installed-version.mjs" || true` ——
**信息性**，不参与成败判定（所以日志里那 4 个 `FAIL` 不会让 `TESTRUN_EXIT` 变非 0）。

`run-tests.js` 新增 **T20 系列 5 项**源码级守卫，防止这个脚本被改坏或被遗忘：

| 断言 | 为什么 |
|---|---|
| T20a 脚本存在 | —— |
| T20b `test-run.sh` 必须调用它，且用 `\|\| true` 保持信息性 | 否则「本机装的是哪版」又只能靠肉眼猜 |
| T20c **必须保持只读**（不得出现任何写文件 API） | 它跑在用户**真实**的安装目录上，写一下就是事故 |
| T20d 读注册表必须带 PowerShell 降级 | 本机 `reg.exe` 在沙箱黑名单里，直接判 FAIL 会把「读不到」误报成「版本不对」 |
| T20e 打时间不得用 `toISOString()` | UTC 比北京时间少 8 小时，`17:15` 会显示成 `09:15`，看着像旧文件 |

> ⚠️ T20e / T20c 断言前同样**先剥注释** —— 脚本头部特意写了 `toISOString()` 来说明「为什么不能用」，
> 不剥会被当成违规用法（T17 / T19 踩过同一个坑）。

**最终复跑**：`bash _test_daemon/test-run.sh` → `TESTRUN_EXIT=0`，**耗时 1m27s**，
`run-tests.js` **123/123**、no-disturb **16/16**、布局滚动实测 **14/14**、插件面板实测 **47/47**。
日志里唯一的 4 个 `FAIL` 全部来自那条信息性步骤。47999 端口已释放；
`data4/daemon.log` 全程**没有** `/api/switch`（测试禁区未被触碰）。
