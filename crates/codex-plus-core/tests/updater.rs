...
fn github_payload_selects_platform_installer() {
    let release = release_from_github_payload(&json!({
        "tag_name": "v1.0.9",
        "html_url": "https://github.com/luoda2023/LDCodex/releases/tag/v1.0.9",
        "body": "fixes",
        "assets": [
    ...
    if cfg!(windows) {
        assert_eq!(
            ...
            Some("LDCodex_1.0.9_x64-setup.exe")
        );
    } else if cfg!(target_os = "macos") {
        assert_eq!(
            release.asset_name.as_deref(),
            Some("LDCodex_1.0.9_x64.dmg")
        );
    } else {
...
fn latest_json_payload_selects_platform_installer_without_github_api_shape() {
    let release = release_from_latest_json_payload(&json!({
        "version": "v1.1.6",
        "url": "https://github.com/luoda2023/LDCodex/releases/tag/v1.1.6",
        "body": "静态更新描述",
        "assets": [
            {"name": "source.zip", "url": "https://example.test/source.zip"},
            {"name": "LDCodex-1.1.6-windows-x64-setup.exe", "url": "https://example.test/setup.exe"},
            {"name": "LDCodex-1.1.6-macos-x64.dmg", "url": "https://example.test/app.dmg"}
        ]
    }))
    .unwrap();
    ...
    if cfg!(windows) {
        assert_eq!(
            release.asset_name.as_deref(),
            Some("LDCodex-1.1.6-windows-x64-setup.exe")
        );
    } else if cfg!(target_os = "macos") {
        assert_eq!(
            release.asset_name.as_deref(),
            Some("LDCodex-1.1.6-macos-x64.dmg")
        );
    } else {
        assert_eq!(release.asset_name.as_deref(), None);
    }
...
fn asset_selection_prefers_current_platform_artifacts() {
    let assets = vec![
        ...
        (
            "LDCodex_1.0.9_x64-setup.exe".to_string(),
            "https://example.test/setup.exe".to_string(),
        ),
        (
            "LDCodex_1.0.9_x64.dmg".to_string(),
    ...
    if cfg!(windows) {
        ...
        assert_eq!(selected.name, "LDCodex_1.0.9_x64-setup.exe");
    } else if cfg!(target_os = "macos") {
        let selected = select_update_asset(&assets).unwrap();
        assert_eq!(selected.name, "LDCodex_1.0.9_x64.dmg");
    } else {
