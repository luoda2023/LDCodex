...
|---|---|
| 大小 | 53,266,693 字节 |
| 出包时间 | 2026-09-13 23:31:19（`npm run build` 完整构建，5m14s，`BUILD_EXIT=0`） |
| SHA-256 | `6895340C99470BA1C96531FA12C43C09A656DCEDF9B281107786CEB16AF89A56` |

> 这一版是**从源码一路 `npm run build` 出来的**，不是手工 `makensis` 重编的 ——
> 两个自杀修复（⑧ 安装器杀自己、⑨ 卸载器杀安装器）都已编进包里。
> 装完实测：`/S /UPDATE` **14.4 秒、ExitCode 0**、`DisplayVersion = 88.8.4`、核验 **7/7 ✅**、测试 **127/127 ✅**。
